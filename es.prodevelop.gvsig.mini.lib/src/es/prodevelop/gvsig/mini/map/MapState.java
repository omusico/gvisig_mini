...
/**
 * Class to persist phisically the map state between sesions. The state is
 * persisted into sdcard/gvsig/Utils.CONFIG_DIR/mapstate.txt
 * 
 * @author aromeu
 * @author rblanco
 * 
...
public class MapState {
	...
	private final static Logger log = Logger
			.getLogger(MapState.class.getName());
	 ...
	 * @param map
	 */
	public MapState(IMapView mapView) {
		try {
			...
			dirPath = SDDIR + File.separator + appDir + File.separator
	...
	public synchronized void persist() {
		try {
			if (mapView != null) {
				...
				log.log(Level.FINE,
						"Persist map state to: " + f.getAbsolutePath());
				if (!f.exists()) {
				...
				out.write(ZOOM + "=" + mapView.getZoomLevel() + "\n");
				out.write(LAYER + "="
						+ mapView.getMRendererInfo().getFullNAME() + "\n");

				out.write(X + "="
						+ mapView.getMRendererInfo().getCenter().getX() + "\n");
				out.write(Y + "="
						+ mapView.getMRendererInfo().getCenter().getY() + "\n");
	...
	public synchronized boolean load() {
		...
		try {
			...
			int zoom = Integer.valueOf(properties.get(ZOOM).toString())
					.intValue();
			String layer = properties.get(LAYER).toString();
			String gvTilesPath = properties.get(GVTILES).toString();

			if (gvTilesPath.compareTo("") != 0
					&& gvTilesPath.compareToIgnoreCase("null") != 0) {
				log.log(Level.FINE, "gvtilesPath: " + gvTilesPath);
				this.gvTilesPath = (gvTilesPath);
				Layers.getInstance().loadProperties(this.gvTilesPath);
			}
			mapView.onLayerChanged(layer);
			mapView.getMapViewController().setMapCenter(x, y);
			mapView.getMapViewController().setZoomLevel(zoom);
