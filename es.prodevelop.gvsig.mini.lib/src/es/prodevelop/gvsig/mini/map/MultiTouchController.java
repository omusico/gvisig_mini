 ...
 * 
 * Released under the Apache License v2 and GPL v2 or at your option any later version.
 */
...
public class MultiTouchController<T> {
	...
	// events
	// (larger dx/dy events are ignored) -- helps eliminate jumping on finger 2
	// up/down
	...
	// multitouchHeight between
	// multitouch events (larger-jump events are ignored) -- helps eliminate
	// jumping on finger 2 up/down
	...
	// Conversion from object coords to screen coords, and from drag width to
	// object scale
	...
	static {
		...
		try {
			...
			m_findPointerIndex = MotionEvent.class.getMethod(
					"findPointerIndex", Integer.TYPE);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		try {
			...
			// Handle history first, if any (we sometimes get history with
			// ACTION_MOVE events)
			int histLen = event.getHistorySize() / pointerCount;

			// Don't try to fetch second touchpoint if only one exists
			int secondPointerIndex = (Integer) m_findPointerIndex.invoke(event,
					1);

			for (int i = 0; i < histLen; i++) {
				float pressureI = (Float) m_getPressure.invoke(event, i);
				if (secondPointerIndex >= 0) {
					float historicalX = (Float) m_getHistoricalX.invoke(event,
							secondPointerIndex, i);
					float historicalY = (Float) m_getHistoricalY.invoke(event,
							secondPointerIndex, i);
					float historicalPressure = (Float) m_getHistoricalPressure
							.invoke(event, secondPointerIndex, i);
					decodeTouchEvent(event.getHistoricalX(i), event
							.getHistoricalY(i), pressureI, pointerCount,
							historicalX, historicalY, historicalPressure,
							MotionEvent.ACTION_MOVE, true, event
									.getHistoricalEventTime(i));
				} else {
					// Can't read from invalid pointer index
					decodeTouchEvent(event.getHistoricalX(i), event
							.getHistoricalY(i), pressureI, pointerCount, 0, 0,
							0.0f, MotionEvent.ACTION_MOVE, true, event
									.getHistoricalEventTime(i));
				}
			}
			...
			if (secondPointerIndex >= 0) {
				float pressure = (Float) m_getPressure.invoke(event,
						secondPointerIndex);
	...
	private void resetDrag() {
		...
		float scaleInv = (objPosAndScale.scale == 0.0f ? 1.0f
				: 1.0f / objPosAndScale.scale);
		objDraggedPointX = (currPt.getX() - objPosAndScale.xOff) * scaleInv;
		objDraggedPointY = (currPt.getY() - objPosAndScale.yOff) * scaleInv;

		// Figure out ratio between object scale factor and multitouch diameter
	...
	private void performDrag() {
		...
		float scale = (objPosAndScale.scale == 0.0f ? 1.0f
				: objPosAndScale.scale);
		float newObjPosX = currPt.getX() - objDraggedPointX * scale;
		float newObjPosY = currPt.getY() - objDraggedPointY * scale;

		// Get new drag diameter (avoiding divsion by zero), and calculate new
		// drag scale
		float diam;
		...
		// Get the new obj coords and scale, and set them (notifying the
		// subclass of the change)
		objPosAndScale.set(newObjPosX, newObjPosY, newScale);
		boolean success = objectCanvas.setPositionAndScale(draggedObject,
				objPosAndScale, currPt);
		if (!success)
	...
	private void multiTouchController() {
		...
		case MODE_DRAG:
			...
			if (!currPt.isDown()) {
				...
				resetDrag();
				// Need to let events settle before moving things, to help with
				// event noise on touchdown
				dragStartTime = currPt.getEventTime();
				dragSettleTime = dragStartTime + EVENT_SETTLE_TIME_INTERVAL;

			} else {
				// Point 1 is still down and point 2 did not change state, just
				// do single-point drag to new location
		...
		case MODE_STRETCH:
			...
			if (!currPt.isMultiTouch() || !currPt.isDown()) {
				...
				if (Math.abs(currPt.getX() - prevPt.getX()) > MAX_MULTITOUCH_POS_JUMP_SIZE
						...
						|| Math.abs(currPt.getMultiTouchWidth()
								- prevPt.getMultiTouchWidth()) * .5f > MAX_MULTITOUCH_DIM_JUMP_SIZE
						|| Math.abs(currPt.getMultiTouchHeight()
								- prevPt.getMultiTouchHeight()) * .5f > MAX_MULTITOUCH_DIM_JUMP_SIZE) {
	...
	public static class PointInfo {
		...
		private boolean down, isMultiTouch, diameterSqIsCalculated,
		...
		private void set(float x, float y, float pressure, int pointerCount,
			...
			// pointerCount +
			// " x2: " + x2 + " y2: " + y2 + " action: " + action + " down: " +
			// down);
	...
	public static interface MultiTouchObjectCanvas<T> {
		 ...
		 * object at the point, or null if nothing to drag.
		 */
		public T getDraggableObjectAtPoint(PointInfo pt);

		/**
		 * Get the screen coords of the dragged object's origin, and scale
		 * multiplier to convert screen coords to obj coords. Call the .set()
		 * method on the passed PositionAndScale object.
		 */
		public void getPositionAndScale(T obj,
				PositionAndScale objPosAndScaleOut);

		/**
		 * Set the position and scale of the dragged object, in object coords.
		 * Return true for success, or false if those parameters are out of
		 * range.
		 */
		 ...
		 * multitouch is initiated. Also called with null when drag op stops.
		 */
