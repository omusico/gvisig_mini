...
public class ResourceLoader {
	...
	private static int[] markerDrawables = new int[] { R.drawable.marker1,
			R.drawable.marker2, R.drawable.marker3, R.drawable.marker4,
			R.drawable.marker5, R.drawable.marker6, R.drawable.marker7,
			R.drawable.marker8, R.drawable.marker9 };
	...
	public static void addResources(ArrayList ids) {
		if (ids == null)
			return;
		final int size = ids.size();

		for (int i = 0; i < size; i++) {
			imgs.put(ids.get(i), BitmapFactory.decodeResource(
					context.getResources(), (Integer) ids.get(i)));
		}
	}

	public static void addResource(String key, Integer id) {
		imgs.put(key, BitmapFactory.decodeResource(context.getResources(), id));
	}

	public static Bitmap getPointSymbolDrawable() {
		if (markerCounter == 9)
			markerCounter = 1;
		int marker = markerCounter++ % 9;

		return BitmapFactory.decodeResource(context.getResources(),
				markerDrawables[marker]);
	}

	public static Bitmap getPointSymbolDrawableYellow() {
		return BitmapFactory.decodeResource(context.getResources(),
				R.drawable.marker0);
	}
}
