 ...
 * @author aromeu
 * @author rblanco
 * @deprecated Build a brand new POIProxyOverlay
 * 
 */
public class NameFinderOverlay extends MapOverlay {
	...
	public NameFinderOverlay(Context context, IMapView mapView, String name) {
...
//			}
	...
	public Feature getNearestFeature(Pixel pixel) {
		try {
//			boolean found = false;
//			final NamedMultiPoint nameds = getIMapView().map.nameds;
//			if (nameds == null)
//				return null;
...
//
//				int[] coords = getIMapView().getMRendererInfo().toPixels(
...
//			}
	...
	public ItemContext getItemContext() {
		return null;
//		return new POIContext(getIMapView().map);
	}
...
//				m.obj = ((Named) getIMapView().map.nameds.getPoint(indexPOI)).description;
//				getIMapView().map.getMapHandler().sendMessage(m);
//			}
//			return true;
		} catch (Exception ex) {
