...
public class LocationTimer extends TimerTask {
	...
	private final static Logger logger = Logger.getLogger(LocationTimer.class
			.getName());
	...
	public LocationTimer(LocationHandler locationHandler) {
		try {
			...
			map = (IMapActivity) locationHandler.mContext;
		} catch (Exception e) {
	...
	public void schedule(int period) {
		try {
			logger.log(Level.FINE, "Scheduling GPSTimerTask with a period of: "
					+ period);
			timer = new Timer();
