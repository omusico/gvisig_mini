...
public class ShowStreetView extends Functionality {
	...
	public ShowStreetView(IMapActivity map, int id) {
		super(map, id);
		try {
	...
	public boolean execute() {
		try {			
			String uri = "google.streetview:cbll=" + getMap().getMapView().getCenterLonLat()[1]  + "," 
			                         + getMap().getMapView().getCenterLonLat()[0]       
			                         + "&cbp=1,0,,0,1&mz=" + getMap().getMapView().getZoomLevel(); 
			Intent intent = new Intent(); 
			intent.setData(Uri.parse(uri)); 
			intent.setAction("android.intent.action.VIEW"); 
			((Activity)getMap()).startActivity(intent);
