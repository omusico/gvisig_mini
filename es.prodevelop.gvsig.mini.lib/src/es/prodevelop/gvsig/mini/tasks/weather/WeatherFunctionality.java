...
public class WeatherFunctionality extends Functionality {
	...
	public WeatherFunctionality(IMapActivity map, int id) {		
		super(map, id);		
		double[] center = getMap().getMapView().getCenterLonLat();
		this.lon = center[0];
		this.lat = center[1];
	...
	public WeatherFunctionality(IMapActivity map, int id, double lat, double lon) {
		super(map, id);
		this.lat = lat;
		this.lon = lon;
		WeatherHandler handler = new WeatherHandler();
	...
	private class WeatherHandler extends TaskHandler {
		...
		public void handleMessage(final Message msg) {
			...
			case TaskHandler.CANCELED:
				getMap().getMapHandler().sendEmptyMessage(IMapActivity.WEATHER_CANCELED);
				break;
			case TaskHandler.FINISHED:
				getMap().getMapHandler().sendEmptyMessage(IMapActivity.WEATHER_SHOW);
				break;
			case TaskHandler.INITED:
				getMap().getMapHandler().sendEmptyMessage(IMapActivity.WEATHER_INITED);
				break;
			case TaskHandler.ERROR:
				getMap().getMapHandler().sendEmptyMessage(IMapActivity.WEATHER_ERROR);
				break;
