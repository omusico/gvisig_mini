...
public class LayersActivity extends ExpandableListActivity {
	...
	public void showGetCapabilitiesAlert() {
		try {
			...
			input.setText("http://www.idee.es/wms/PNOA/PNOA");
//			 input.setText("http://ovc.catastro.meh.es/Cartografia/WMS/ServidorWMS.aspx");
			...
			for (int i = 0; i < 3; i++) {
				...
				if (layersList == null) {
					...
					for (int j = 0; j < length; j++) {
						...
						curChildMap.put(
								LAYER,
								processLayerNameToDisplay(layersList.elementAt(
										j).toString()));
					}
				}
	...
	public String processLayerNameToDisplay(String layerName) {
		try {
			if (layerName.contains(MapRenderer.NAME_SEPARATOR)) {
				String[] parts = layerName.split(MapRenderer.NAME_SEPARATOR);
				String result = parts[1] + MapRenderer.NAME_SEPARATOR
						+ parts[0];
				return result;
			} else {
				return layerName;
		...
		try {
			...
			if (groupPosition == 3) {
				...
				if (layers) {
					...
					String s = extractLayerNameFromTextView(t.getText1()
							.getText().toString());
					log.log(Level.FINE, "layer clicked: " + s);
					Intent i = new Intent();
					if (getIntent().getBooleanExtra(FROM_FILE_EXPLORER, false)) {
						i = new Intent(this, Utils.DEFAULT_MAP_CLASS);
						i.putExtra("layer", s);
	...
	public String extractLayerNameFromTextView(String text) {
		try {
			if (text.contains(MapRenderer.NAME_SEPARATOR)) {
				String[] parts = text.split(MapRenderer.NAME_SEPARATOR);
				String result = parts[1] + MapRenderer.NAME_SEPARATOR
						+ parts[0];
				return result;
			} else {
