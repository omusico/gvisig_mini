 ...
 * @author aromeu
 * @author rblanco
 * @deprecated Use ILegend instead and delete this.....
 * 
 */
public class AndroidGeometryDrawer implements IGeometryDrawer {
	...
	protected static android.graphics.Point START_SPOT;
	protected static android.graphics.Point FINISH_SPOT;
	protected static android.graphics.Point POIS_SPOT;
	...
	protected static Pixel firstRoutePixel, oldFirstRoutePixel, lastRoutePixel,
			oldLastRoutePixel;
	protected static ArrayList<Pixel> pixelsRoute;
	...
	public AndroidGeometryDrawer(IMapView t, Context con) {
		try {
	...
	public void draw(Point p, Object graphics, Extent extent, Object viewPort) {
		try {
			if (mustDraw(extent, p)) {
				...
				int[] coords = mapView.getMRendererInfo().toPixels(
	...
	public void drawpoi(Point p, Object graphics, Extent extent, Object viewPort) {
		try {
			if (mustDraw(extent, p)) {
				...
				// .getCRS(mapView.getMRendererInfo().getSRS()));
				int[] coords = mapView.getMRendererInfo().toPixels(
						new double[] { p.getX(), p.getY() });
	...
	public static void drawstart(Point p, Object graphics, Extent extent,
			Object viewPort, IMapView mapView) {
		try {
			final Canvas c = (Canvas) graphics;
			final MapRenderer renderer = mapView.getMRendererInfo();
			final double[] xy = ConversionCoords.reproject(p.getX(), p.getY(),
	...
	public void drawend(Point p, Object graphics, Extent extent, Object viewPort) {
		try {
			...
			final double[] xy = ConversionCoords.reproject(p.getX(), p.getY(),
					CRSFactory.getCRS("EPSG:4326"),
					CRSFactory.getCRS(renderer.getSRS()));
			// int[] coords = mapView.getMRendererInfo().toPixels(new
	...
	public static void drawend(Point p, Object graphics, Extent extent,
			Object viewPort, IMapView mapView) {
		try {
			final Canvas c = (Canvas) graphics;
			final MapRenderer renderer = mapView.getMRendererInfo();
	...
	public static void draw(MultiLineString arg0, Object arg1, Extent arg2,
			Object arg3, IMapView mapView) {
		try {
			final LineString[] lineStrings = arg0.getLineStrings();

			final int size = lineStrings.length;
			for (int i = 0; i < size; i++) {
				draw(lineStrings[i], arg1, arg2, arg3, mapView);
			}
		} catch (Exception e) {
			log.log(Level.SEVERE, "drawMultiLine: ", e);
		}
	...
	public static void draw(LineString arg0, Object arg1, Extent arg2,
			Object arg3, IMapView mapView) {
		try {
			final double[] xCoords = arg0.getXCoords();
			final double[] yCoords = arg0.getYCoords();
			...
			if ((oldFirstRoutePixel != null)
				...
				for (int i = 0; i < size; i++) {
					pixelsRoute
							.set(i,
									new Pixel(
											pixelsRoute.get(i).getX()
													- (oldFirstRoutePixel
															.getX() - firstRoutePixel
															.getX()),
											pixelsRoute.get(i).getY()
													- (oldFirstRoutePixel
															.getY() - firstRoutePixel
															.getY())));
				}
