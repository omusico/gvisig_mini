...
public class YOURSFunctionality extends Functionality {
	...
	public YOURSFunctionality(IMapActivity map, int id) {
		super(map, id);
	...
	private class YOURSHandler extends TaskHandler {
		...
		public void handleMessage(final Message msg) {
			...
			case TaskHandler.CANCELED:
				getMap().getMapHandler().sendEmptyMessage(IMapActivity.ROUTE_CANCELED);
				break;
			case TaskHandler.FINISHED:
				getMap().getMapHandler().sendEmptyMessage(IMapActivity.ROUTE_SUCCEEDED);
				break;
			case TaskHandler.INITED:
				getMap().getMapHandler().sendEmptyMessage(IMapActivity.ROUTE_INITED);
				break;
			case TaskHandler.ERROR:
				getMap().getMapHandler().sendEmptyMessage(
						IMapActivity.ROUTE_NO_CALCULATED);
				break;
			case TaskHandler.NO_RESPONSE:
				getMap().getMapHandler()
						.sendEmptyMessage(IMapActivity.ROUTE_NO_RESPONSE);
				break;
			case TaskHandler.BAD_RESPONSE:
				getMap().getMapHandler().sendEmptyMessage(
						IMapActivity.ROUTE_NO_CALCULATED);
				break;
			case IMapActivity.ROUTE_ORIENTATION_CHANGED:
				// route.deleteRoute();
				// cancel();
				break;
			}
