...
import es.prodevelop.gvsig.mini.activities.filter.FeatureFilter;
...
import es.prodevelop.gvsig.mini.views.MapPreview;
...
public class FilteredLazyAdapter extends BaseAdapter implements Filterable,
	...
	public View getView(final int arg0, View convertView, ViewGroup arg2) {
		...
		String desc = "Esto es una prueba";
		...
		if (centerXY[0] == 0 && centerXY[1] == 0)
			...
			double distance = 0;
			try {
				centroid = p.getGeometry().getGeometry().getCentroid();
				distance = Calculator.latLonDist(centroid.getX(),
	...
	public Filter getFilter() {
		if (mFilter == null) {
			mFilter = new FeatureFilter(activity);
		}
		return mFilter;
	}
	...
	public void onDestroy() {
		try {
			...
			// category = null;
			//
			// if (mIndexer != null && mIndexer instanceof BaseIndexer)
			// ((BaseIndexer) mIndexer).destroy();
			// mIndexer = null;
			//
			// if (defaultIndexer != null && mIndexer instanceof BaseIndexer)
			// ((BaseIndexer) defaultIndexer).destroy();
			// defaultIndexer = null;
		} catch (Exception e) {
