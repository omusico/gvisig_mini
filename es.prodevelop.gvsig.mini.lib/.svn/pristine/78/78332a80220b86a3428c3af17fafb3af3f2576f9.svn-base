...
public class ViewSimpleLocationOverlay extends MapOverlay {
	...
	public ViewSimpleLocationOverlay(final MapActivity mapActivity, final IMapView mapView,
			String name) {
		super(mapActivity, mapView, name);
	...
	private void drawOrientation(Canvas c, IMapView osmv, int[] coords) {
		try {
			try {
				// if (!noOrientation)
				// return;
				if (!Settings.getInstance().getBooleanValue(
						getContext().getText(
								R.string.settings_key_orientation).toString())) {
	...
	@Override
	public void onDraw(final Canvas c, final IMapView osmv) {
		try {
			
			if (this.panControl == null) {
				this.panControl = (PanControl)this.getMapView().getControlByName(PanControl.name);
			}

			if (!mapActivity.isLocationHandlerEnabled())
				return;
...
//				if (this.panControl != null) {
//					coords[0] += this.panControl.getOffsetX();
//					coords[1] += this.panControl.getOffsetY();
//				}
				...
				if (this.mLocation.acc != 0) {
					...
					if (CRSFactory.getCRS(renderer.getSRS()).getUnitsAbbrev()
						...
						if (mapView.isNavigationMode())
							mapView.adjustViewToAccuracyIfNavigationMode(mLocation.acc);
	...
	public Feature getNearestFeature(Pixel pixel) {
		try {
			...
			// mapView.getMRendererInfo().reprojectGeometryCoordinates(p,
			// "EPSG:4326");
			int[] pxy = mapView.getMRendererInfo().toPixels(co);
	...
	public ItemContext getItemContext() {
		return new GPSItemContext(mapActivity);
	}
	...
	public boolean onSingleTapUp(MotionEvent e, IMapView osmtile) {
		try {
			...
			if (!found)
				...
				mapActivity.getMapHandler().sendMessage(m);
			}
			return true;
		} catch (Exception ex) {
	...
	public Point getLocationLonLat() {
		if (mLocation == null
				|| !mapActivity.isLocationHandlerEnabled())
